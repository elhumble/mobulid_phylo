...
## Plink .raw files created with bash script ##

## Read in the data into genlight objects ##
## and remove any loci that are entirely unsampled (because these are subsetted from the whole mobulidae dataset) ##
## finally, assign species names to dataset@pop, and geographic information (where relevant) to ocean ##
...
mantas <- read.PLINK("data/p10_mantas.raw", n.cores = 1)

#~~ remove unsampled loci
toRemove_mantas <- is.na(glMean(mantas, alleleAsUnit = FALSE)) # TRUE where NA
which(toRemove_mantas) # position of entirely non-typed loci
mantas_data <- mantas[, !toRemove_mantas]

#~~ to see how many SNPs retained and missing data
mantas_data

#~~ assign species info
ids_mantas <- read.table("data/IDs_p10_mantas.txt")
species_mantas <- as.factor(ids_mantas$V1)
mantas_data@pop <- species_mantas

#~~ assign geography info
geog_mantas <- read.table("data/IDs_p10_mantas_geog.txt")
ocean_mantas <- as.factor(geog_mantas$V1)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 
#      Mobular / Japanica      #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 

mobjap <- read.PLINK("data/p10_mobjap.raw", n.cores = 1)

#~~ remove unsampled loci
toRemove_mobjap <- is.na(glMean(mobjap, alleleAsUnit = FALSE)) # TRUE where NA
which(toRemove_mobjap) # position of entirely non-typed loci
mobjap_data <- mobjap[, !toRemove_mobjap]

#~~ to see how many SNPs retained and missing data
mobjap_data

#~~ assign species info
ids_mobjap <- read.table("data/IDs_p10_mobjap.txt")
species_mobjap <- as.factor(ids_mobjap$V1)
mobjap_data@pop <- species_mobjap

#~~ assign geography info
geog_mobjap <- read.table("data/IDs_p10_mobjap_geog.txt")
ocean_mobjap <- as.factor(geog_mobjap$V1)


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 
#      Thurstoni / Kuhlii / Eregoo       #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 

thurerekuh <- read.PLINK("data/p10_thurerekuh.raw", n.cores = 1)

#~~ remove unsampled loci
toRemove_thurerekuh <- is.na(glMean(thurerekuh, alleleAsUnit = FALSE)) # TRUE where NA
which(toRemove_thurerekuh) # position of entirely non-typed loci
thurerekuh_data <- thurerekuh[, !toRemove_thurerekuh]

#~~ to see how many SNPs retained and missing data
thurerekuh_data

#~~ assign species info
ids_thurerekuh <- read.table("data/IDs_p10_thurerekuh.txt")
species_thurerekuh <- as.factor(ids_thurerekuh$V1)
thurerekuh_data@pop <- species_thurerekuh

#~~ assign geography info
geog_thurerekuh <- read.table("data/IDs_p10_thurerekuh_geog.txt")
ocean_thurerekuh <- as.factor(geog_thurerekuh$V1)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 
#      Hypostoma / Munkiana      #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 

hypmunk <- read.PLINK("data/p10_hypmunk.raw", n.cores = 1)

#~~ remove unsampled loci
toRemove_hypmunk <- is.na(glMean(hypmunk, alleleAsUnit = FALSE)) # TRUE where NA
which(toRemove_hypmunk) # position of entirely non-typed loci
hypmunk_data <- hypmunk[, !toRemove_hypmunk]

#~~ to see how many SNPs retained and missing data
hypmunk_data

#~~ assign species info
ids_hypmunk <- read.table("data/IDs_p10_hypmunk.txt")
species_hypmunk <- as.factor(ids_hypmunk$V1)
hypmunk_data@pop <- species_hypmunk

#~~ assign geography info
geog_hypmunk <- read.table("data/IDs_p10_hypmunk_geog.txt")
ocean_hypmunk <- as.factor(geog_hypmunk$V1)
...
a <- ggplot(filter(ggplot_mob, pc == 12), 
  ...
  theme(axis.text.x = element_text(face = "plain"),
        ...
        strip.text = element_text(size=9, hjust = 0), # 13
...
b <- ggplot(filter(ggplot_mob, pc == 13), 
  ...
  labs(color="Species") +
  labs(shape="Ocean") + 
  facet_wrap(~ plot, scales = "free", ncol = 1, labeller = label_parsed) + 
...
png(file="figs/Figure_3.png", units = "in", res = 300, height=11, width=9)
a+b
dev.off()
...
png(file="figs/Figure_3A.png", units = "in", res = 300, height=11, width=7)
...
png(file="figs/Full_PCA.png", units = "in", res = 300, height=5, width=13)
x + y
dev.off()
...
png(file="figs/Figure_S2.png", units = "in", res = 300, height=10, width=9)
eig_plot_a + eig_plot_b
dev.off()
